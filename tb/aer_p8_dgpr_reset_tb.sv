...
// Reset-specific verification for the split-reset Pareto candidate.
module aer_p8_dgpr_reset_tb;
    logic clk;
    logic clock_enable;
    logic rst_n;
    ...
    task automatic fail(input string message);
        begin
            errors = errors + 1;
            $display("P8_DGPR_RESET_ASSERT_FAIL message=%s", message);
        end
    endtask
    ...
    initial begin : test_sequence
        ...
        // ACK/out_valid clear asynchronously even without a clock.  Internal
        // state remains masked until the two release edges synchronously clear it.
        #1 rst_n = 1'b0;
        ...
        @(posedge clk);
        #1 check_safe_low("second release clock");

        // Build an active ACK and stalled output, stop the clock, and assert
        // reset between edges.  External state must clear immediately.
        @(negedge clk);
        out_ready = 1'b0;
        ...
        // Source 4 stays high across reset; source 9 starts during reset.
        src_req_async[9] = 1'b1;
        #20 rst_n = 1'b1;
        #20 check_safe_low("mid-phase clockless release remains reset");
        ...
        $display("METRIC p8_dgpr_clockless_reset_checks=4");
        $display("METRIC p8_dgpr_restart_transfers=%0d", transfer_count - base_transfer_count);
        $display("METRIC p8_dgpr_reset_errors=%0d", errors);
        if (errors == 0)
            $display("P8_DGPR_RESET_TEST_PASS");
        else
            $display("P8_DGPR_RESET_TEST_FAIL");
        #20;
        $finish;
    end
