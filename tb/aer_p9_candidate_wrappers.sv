...
`ifdef P9_LOOP_REDUCE
    aer_pending_direct_gray_scr_loop_reduce implementation (.*);
`elsif P9_ONEHOT_DECODE
    aer_pending_direct_gray_scr_onehot_decode implementation (.*);
`else
    aer_pending_direct_gray_scr_onehot_tree implementation (.*);
`endif
endmodule
...
module aer_source_resident_dut (
    ...
    aer_p9_selected_dut implementation (.*);
endmodule
