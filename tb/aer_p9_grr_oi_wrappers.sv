...
);aer_pending_gray_rank_reuse_operand_isolated implementation(.*);endmodule
module aer_improved_hybrid#(parameter integer NUM_SOURCES=16,ADDR_W=4)(
 ...
 output logic[NUM_SOURCES-1:0]src_ack_async,output logic[ADDR_W-1:0]out_addr,
 output logic out_valid,input logic out_ready
);aer_pending_gray_rank_reuse_operand_isolated implementation(.*);endmodule
