...
P7-GE는 P4-C의 입력·출력 규격, 이벤트 저장 능력과 최대 처리율을 유지하면서 10-bit 순번 상태를 4-bit Gray epoch로 줄였다. 최종 제안 구조 P8-DG-SCR은 여기서 기능을 더 줄이지 않고 Gray 상태, 선택 tree와 reset 배포 구조를 함께 다시 설계한다. 서버 제공 FPR 180 nm reference kit의 동일 조건에서 P7-GE보다 post-route 셀 면적을 5.03%, 기본 활동률 전력 추정치를 4.58% 줄였고 clock-tree·timing·DRC·연결 검사를 통과했다.
...
P3 등 중간 설계의 세부 결과는 `results/`에 보존하였다. 본 보고서에서는 전통적 동작을 보여주는 T0-PPA, 기능 기반을 만든 P4-C, 공정한 Gray 중재를 완성한 P7-GE와 최종 PPA 개선본 P8-DG-SCR을 중심으로 설명한다.
...
서로 다른 단계의 ACK를 같은 의미로 해석하면 안 된다. T0-PPA와 P7-GE/P8-DG-SCR은 source 쪽 요청 규약을 공유하지만 receiver 쪽 출력 규약은 다르다.

#### 2.1 Source와 controller 사이

| 신호 | T0-PPA에서의 의미 | P7-GE/P8-DG-SCR에서의 의미 |
|---|---|---|
...
#### 2.3 P7-GE/P8-DG-SCR과 receiver 사이

P7-GE와 P8-DG-SCR은 동기식 후단 모듈에 연결하기 위해 `out_addr`, `out_valid`, `out_ready`를 사용한다. Controller가 유효한 주소를 제시하면 `out_valid=1`이 되고, receiver가 받을 준비가 되면 `out_ready=1`이 된다. 두 신호가 함께 1인 clock edge에서 이벤트 하나가 전달된다.
...
| P7-GE 대비 PPA 개선 | Post-route area -5.03%, vectorless power -4.58% | 동일 저장·출력·순서와 legal clock tree에서 달성 |
| Timing closure | Core setup +3.235 ns, CDC max-delay +0.201 ns, hold +0.028 ns | 10 ns 및 CDC 물리 제약 달성 |
| Reset release | Recovery +9.104 ns, removal +0.043 ns | 유일한 async-reset 2FF 경로 통과 |
| 논리·물리 검증 | LEC state 75/75, DRC/connectivity 0/0 | Core 수준 검증 완료 |
...
P8-DG-SCR은 다음 기술을 하나의 합성 가능한 RTL에 결합한다.

- 비동기 입력을 위한 source별 2FF CDC
- Source별 1-bit pending과 early ACK
- Pending next-state cut-through
- Direct 4-bit Gray epoch
- 공유 pair/quarter/half/grant OR tree와 XOR tournament branch 선택
- Registered valid/ready output
- Async assertion, 2-clock synchronous release와 output isolation
- 2 async-reset FF + 36 resetless FF + 37 synchronous-clear FF의 reset partition

각 기술은 testbench 기능 결과뿐 아니라 Vivado gate simulation, Cadence Xcelium, Genus, Conformal과 Innovus로 이어지는 단계에서 검증하였다.

### 4. 완성도

최종 설계는 RTL·gate·Xcelium의 139-event broad regression, CDC 192-point phase sweep, full-backlog 순서, 64개 random pending mask, worst-position fairness와 clockless/mid-phase reset 시험을 통과했다. Genus netlist의 상태점 75개가 모두 equivalent였고 Innovus post-route에서 setup/hold와 reset recovery/removal timing을 만족했다. DRC와 connectivity 오류는 0이며 실제 post-route 화면과 DEF, SDF, SPEF, netlist를 보존하였다.

전력 결과는 도구 추정임을 분리해 기술했고, timing을 만족하지 못한 fall-through 구조는 기능상 이점이 있어도 최종안에서 제외했다. 달성한 결과와 아직 필요한 실리콘·traffic 기반 검증을 구분하였다.

### 5. 2차 설계과제에서의 재사용

P8-DG-SCR은 특정 응용 계산을 수행하는 회로가 아니라 여러 비동기 event를 하나의 `source_id + valid/ready` 주소 스트림으로 만드는 독립 전단 IP다.

```text
뉴런 또는 센서 이벤트
    → P8-DG-SCR 이벤트 동기화·저장·중재
    → 원래 source ID 출력
...
- P8-DG-SCR의 CDC phase sweep은 디지털 protocol sampling 시험이며 실리콘 MTBF 측정이 아니다.
- Event 보존은 source가 ACK까지 request를 유지하고 source당 한 번에 요청 하나만 제시하는 계약 안에서 성립한다.
- P8-DG-SCR은 FCFS가 아니며 original event timestamp를 전송하지 않는다.
...
- 180 nm 결과는 서버 reference kit의 잠정 비교이며 공식 공정 확정 뒤 P7과 P8을 함께 재실행해야 한다.
...
P4-C는 비동기 요청 동기화, source별 이벤트 보관, early ACK, 공정한 선택과 1 event/clock 출력을 제공하였다. P7-GE는 중재 상태를 4-bit Gray epoch로 줄여 공정성과 주소 전환 절감을 달성했다. P8-DG-SCR은 같은 P7 기능과 출력 순서를 유지한 채 Direct Gray, 공유 선택 tree와 reset partition을 결합했다.

서버 제공 FPR 180 nm reference kit에서 P8은 P7보다 post-route 셀 면적을 5.03%, vectorless power estimate를 4.58% 줄였다. RTLStim2Gate VCD는 9.525% 낮았지만 driver-net coverage 차이 때문에 보조 관찰로만 사용한다. Core setup +3.235 ns, CDC max-delay +0.201 ns, hold +0.028 ns, recovery +9.104 ns와 removal +0.043 ns로 모든 timing을 통과했고 clock-tree·DRC·connectivity 위반은 0이었다. 기능, gate, CDC, reset, Xcelium, LEC와 물리 검증을 모두 통과했으므로 registered-output P8-DG-SCR을 최종 AER 컨트롤러로 채택한다.
