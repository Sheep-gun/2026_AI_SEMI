foreach required_env {AER45_RTL AER45_TOP AER45_TAG AER45_OUT AER45_SYNC_FF} {
...
foreach required_file [list $slow_lib $rtl_file] {
    if {![file exists $required_file]} {error "AER45 input missing: $required_file"}
}
file mkdir $out_dir
set_db init_hdl_search_path [list [file dirname $rtl_file]]
read_libs $slow_lib
...
puts "AER45_SYNCHRONIZER_COUNT=$synchronizer_count expected=$expected_sync"
if {$synchronizer_count != $expected_sync} {
    error "$tag expected $expected_sync synchronizer flops; found $synchronizer_count"
}
if {$synchronizer_count>0} {set_db $synchronizer_insts .preserve true}
syn_opt
...
puts "AER45_GENUS_DONE tag=$tag top=$top"
exit
