...
if {![file exists $rtl_file]} {
    error "P9-OHT RTL not found at repo-relative path: $rtl_file"
}
...
elaborate aer_pending_direct_gray_scr_onehot_tree
...
# rst_n still reaches the asynchronous reset pins of reset_release_q[1:0].
# Do not false-path it: recovery/removal for those two flops must remain timed.
...
# This Genus release does not act on the RTL ASYNC_REG attribute.  Preserve
# exactly the 32 request synchronizer flops and two reset-release flops after
# mapping so syn_opt cannot merge, retime, or remove either stage.
...
puts "P9OHT_SYNCHRONIZER_INSTS_FOUND=$synchronizer_count"
if {$synchronizer_count != 34} {
    error "P9-OHT expected 34 request/reset synchronizer flops after mapping; found $synchronizer_count"
}
set_db $synchronizer_insts .preserve true
puts "P9OHT_SYNCHRONIZER_INSTS_PRESERVED=$synchronizer_count"
...
write_hdl > [file join $input_dir aer_pending_direct_gray_scr_onehot_tree_pnr.v]
puts "P9OHT_GENUS_DONE"
exit
