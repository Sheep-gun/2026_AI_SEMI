...
set vcd_file [file join $project_root sim waves p9_grr_contract.vcd]
set report_dir [file join $project_root reports p9grr_contract_vcd]
set top aer_pending_gray_rank_reuse_sync_core_reset
puts "P9GRR_VCD_PROJECT_ROOT=$project_root"

foreach required_file [list $rtl_file $vcd_file] {
    if {![file exists $required_file]} {
        error "P9-GRR VCD-power input not found: $required_file"
    }
}
...
if {$synchronizer_count != 34} {
    error "P9-GRR VCD flow expected 34 synchronizer flops; found $synchronizer_count"
}
...
read_vcd -static \
    -vcd_scope aer_contract_fairness_tb/dut/u_dut \
    $vcd_file
report_power > [file join $report_dir p9grr_genus_power_vcd.rpt]
write_saif -computed > [file join $report_dir p9grr_mapped_activity.saif]
puts "P9GRR_CONTRACT_VCD_POWER_DONE"
exit
