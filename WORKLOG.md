...
### P9-GRR output-state reuse and physical Pareto closure

- Kept P8's 16-source four-phase input contract, two-FF CDC, early ACK, sixteen pending slots, registered valid/ready output, one address bus, one event/clock peak rate and 16-service-decision fairness bound.
- Explored one-hot selection trees, one-hot consume, binary/Gray cyclic pointers, dual-edge CDC, operand isolation, mask arbiters and three alternative Gray-successor forms. P9-OHT shortened the selection path; P9-GRR reached the 71-FF reachable-state lower bound by reusing the output rank as the strict-cyclic fairness pointer.
- P9-GRR stores request, ACK and pending bits in fixed Gray-rank order. The permutation is wiring, so the selected pending bit is cleared without a source-address feedback decoder; only the registered output rank is Gray-encoded at the interface.
- Re-ran RTL and post-synthesis broad 139/139, CDC 192/192, reset/restart and fixed-demand tests. The selector matched all 16×65,536 preference/mask combinations and the worst persistent-request position was 16 service decisions.
- Conformal passed all 21 primary outputs and 71 state points for the GRR RTL-to-Genus comparison. P8 and OHT also passed full 21-output/75-state sequential equivalence.
- Performed full-clean hold-target sweeps instead of comparing a tuned P9 against an untuned P8. The fair P8 point is 297 instances, 7,364.650 µm² and 0.79657531 mW at target 0.021 ns. OHT closes at 0.012 ns with 290 instances, 7,291.469 µm² and 0.77267187 mW. GRR closes at 0.020 ns with 281 instances, 6,988.766 µm² and 0.77624020 mW.
- Generated mapped SAIF through Genus RTLStim2Gate and applied it to each final post-route database. P8/OHT/GRR power was 0.59663396/0.58959029/0.57886987 mW with primary-input and flop-output annotation at 100% for all three designs.
- Selected P9-GRR as the area/workload-power main design and retained P9-OHT as the vectorless-power/timing Pareto alternative. GRR improves optimized P8 area 5.10%, vectorless power 2.55%, mapped-SAIF power 2.98% and core setup slack by 1.532 ns; all final clock-tree, DRC and connectivity checks are clean.
- Exported native Innovus post-route PNGs for both final P9 points and added bundle-relative Genus, Conformal, Innovus, SAIF-power and capture flows. All 180 nm numbers remain provisional until the organizer confirms the official PDK and corners.
- Reorganized the public explanation into three functional axes rather than four isolated buzzwords: 2FF input synchronization, the pending-plus-output two-stage elastic buffer, and Gray-rank cyclic arbitration with output-pointer reuse. Added a Korean circuit-level guide that starts from T0 and includes physical operation, clock-by-clock flow, PPA cost and limitations.

### P10 paper-derived selector/code sweep and GRR hold-boundary closure

- Implemented Zheng/Yang IPRRA-style parallel binary-tree arbitration and one-/two-XOR partial-Gray rank orderings without changing P9's 71-FF protocol, storage, throughput or fairness contract.
- Exhaustively checked all 1,048,576 last-rank/request-mask combinations for each selector and passed RTL/post-synthesis broad 139/139, CDC 192/192, reset and fixed-demand regressions.
- Rejected IPRRA at 16 inputs: 151 LUT/71 FF/7.359 ns versus P9's 91 LUT/71 FF/5.881 ns, and FPR 180 nm Genus area 6,582.946 µm² versus 6,087.312 µm².
- Rejected partial-Gray X1/X2 as the main RTL because reduced encoder XOR count increased output switching. X1 produced 142 address transitions versus P9's 114 on the 101-event workload; at its hold-optimized 0.013 ns point it remained larger and higher-power than an equivalently retuned P9.
- Closed the previously unexplored P9-GRR lower hold boundary: targets 0.013, 0.012, 0.011, 0.010, 0.009 and 0.008 ns passed, while 0.007 ns failed at -0.001 ns. The new final target is 0.008 ns with +0.001 ns hold/CDC hold.
- The new P9-GRR point uses 263 instances and 6,742.613 µm², with 0.76127733 mW vectorless and 0.57559566 mW mapped-SAIF power. Versus the previous 0.020 ns point, instances/area/vectorless/SAIF improve by 6.41%/3.52%/1.93%/0.57%; overall/core setup are +0.317/+4.844 ns and DRC/connectivity/clock violations remain 0/0/0.
- Re-ran Conformal on the mapped netlist: 21 primary outputs and 71 DFF compare points were all equivalent, with nonequivalent/abort/unknown all zero. Preserved sanitized final reports under `reports/p10_final/` and detailed paper applicability and rejected paths in `results/P10_PAPER_DERIVED_PPA_EXPLORATION_2026-08-21.md`.

### GPDK045 migration and 8x8 scaling experiment

- Froze the complete server-provided GPDK045/GSCLIB045 digital kit as the provisional deadline target after the organizer did not answer the process question. Setup uses slow 0.9 V/125°C; hold uses fast 1.1 V/0°C. The flow uses a 10 ns clock, 0.20/0.02 ns setup/hold uncertainty, 1 ns I/O delay, 0.8 ns CDC pair max delay, QRC extraction and Metal1-Metal9 signal routing.
- Ported T0-PPA to characterized GSCLIB045 TLATX1/DLY4X1 cells. RTL delivered 139/139 events. Genus/Innovus/Conformal completed at 92 instances, 214.092 µm², 0.00212669 mW vectorless power, +4.126 ns max-delay slack, DRC/connectivity 0/0 and 21-output/5-state equivalence.
- Re-synthesized P4-C, P7-GE, P8-DG-SCR, P9-GRR, P9-OHT, P10-IPRRA, X1 and X2 under the same 45 nm constraints and applied their fixed-demand VCDs. P9-GRR, P9-OHT and X1 remained the only 16-source Pareto candidates.
- Corrected a constraint bug that had applied 0.2 ns setup uncertainty to hold/removal. Split setup/hold uncertainty to 0.20/0.02 ns and modeled reset release with 0.10 ns minimum input delay. Expanded Metal10 VDD/VSS ring spacing to 2.0 µm to close two long parallel-run DRC violations.
- Final clean P&R: P9-GRR 263 instances/669.294 µm²/0.01438235 mW SAIF/core +6.824 ns; P9-OHT 278/709.308/0.01377985/+7.555; P10-X1 263/669.978/0.01451839/+7.061. All setup, hold, recovery, removal, clock, DRC, connectivity and LEC checks pass. GRR remains the balanced main and OHT the high-speed/low-power Pareto.
- Implemented 64-source grouped and IPRRA selectors with identical 265-FF controller state and strict-cyclic behavior. Both matched 36,992 selector cases and integrated RTL lockstep. Genus grouped/IPRRA results were 2,421.018/2,727.108 µm², 3.528/4.334 ns and 0.048288/0.048399 mW VCD. IPRRA is dominated at 64 inputs. The grouped clean P&R result is 1,095 instances, 2,733.264 µm², 0.05784201 mW SAIF, core +5.142 ns and DRC/connectivity 0/0.
- Tested GSCLIB045 MBFF and ICG. MBFF or ICG alone did not help. MBFF+ICG reduced Genus VCD power but reversed post-route to 823.878 µm² and 0.01562547 mW, 23.097% area and 8.643% SAIF above basic P9-GRR. Gated-clock-aware Conformal passed all 21 outputs and 71 state points, so rejection is based on physical PPA rather than correctness.
