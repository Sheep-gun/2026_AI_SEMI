...
// P8-X2: P8 sparse-reset transport with a two-XOR cyclic preference code.
// The lower three binary epoch bits use reflected Gray while bit three stays
// binary.  The 16 preferences are still a bijection, so a continuously pending
// source is served within 16 grants.  A full cycle costs 18 address-bit
// transitions instead of binary's 30 or full Gray's 16.
module aer_pending_xor2_sparse_reset (
    input  logic        clk,
    input  logic        rst_n,
    ...
    logic [3:0] epoch_bin_q,epoch_bin_d,epoch_preference;
    logic [1:0] half_valid;
    logic [3:0] quarter_valid;
    logic [7:0] pair_valid;
    logic b3,b2,b1,b0,grant_valid;
    logic [3:0] selected_addr,out_addr_q,out_addr_d;
    logic out_valid_q,out_valid_d,can_load_output;
    integer source_index,pair_index;

    always_comb begin
        ack_d=ack_q;pending_d=pending_q;epoch_bin_d=epoch_bin_q;
        out_addr_d=out_addr_q;out_valid_d=out_valid_q;
        ...
        epoch_preference={epoch_bin_q[3],epoch_bin_q[2],
                          epoch_bin_q[1]^epoch_bin_q[2],
                          epoch_bin_q[0]^epoch_bin_q[1]};
        ...
        b3=half_valid[epoch_preference[3]]?epoch_preference[3]:
                                                ~epoch_preference[3];
        b2=quarter_valid[{b3,epoch_preference[2]}]?epoch_preference[2]:
                                                        ~epoch_preference[2];
        b1=pair_valid[{b3,b2,epoch_preference[1]}]?epoch_preference[1]:
                                                     ~epoch_preference[1];
        b0=candidate[{b3,b2,b1,epoch_preference[0]}]?epoch_preference[0]:
                                                        ~epoch_preference[0];
    ...
    // Resetless CDC data flops sample throughout reset and are quarantined by
    // the two-clock core reset release, exactly as in P8-SR.
    always_ff @(posedge clk)begin
        req_meta_q<=src_req_async;
        req_sync_q<=req_meta_q;
    end

    // Address data is irrelevant whenever reset forces out_valid low.
    always_ff @(posedge clk)out_addr_q<=out_addr_d;

    always_ff @(posedge clk or negedge core_rst_n)begin
        if(!core_rst_n)begin
            ack_q<='0;pending_q<='0;epoch_bin_q<='0;out_valid_q<=1'b0;
        end else begin
            ack_q<=ack_d;pending_q<=pending_d;epoch_bin_q<=epoch_bin_d;
