...
// P3 area/power-oriented AER transport.
// P2's CDC, parallel 4x4 round-robin and elastic output are retained, while
// each depth-2 event count is replaced by one pending bit. A source whose slot
// is occupied simply holds its four-phase request until backpressure clears.
    ...
    always_comb begin
        ...
        for (group_iter = 0; group_iter < NUM_GROUPS; group_iter = group_iter + 1) begin
            for (local_offset = 0; local_offset < GROUP_SIZE; local_offset = local_offset + 1) begin
                ...
                if (!local_valid[group_iter] && pending_q[source_candidate]) begin
        ...
        // A request is acknowledged only after its single pending slot accepts
        // the event. If occupied, the source naturally sees backpressure.
        ...
        if (can_load_output) begin
            ...
            if (select_valid) begin
                ...
                local_rr_d[select_group] = (select_local == GROUP_SIZE-1) ? '0 : select_local + 1'b1;
            end else begin
                out_valid_d = 1'b0;
            end
        end
