...
// P8-DG-PR experiment: reset-Pareto form of the shared-tree/direct-Gray core.
//
// Externally observable control state (ACK and out_valid) retains asynchronous
// safe-low assertion.  Internal pending and epoch state use synchronous clear;
// both robust release edges clear them before core_rst_n can rise.  Request CDC
// stages and out_addr remain resetless.  This avoids output isolation while
// reducing the asynchronous reset tree from 39 flops in P8-DG-T to 19 flops.
module aer_pending_direct_gray_split_reset (
    input  logic        clk,
    input  logic        rst_n,
    ...
    // Only the externally observed handshake state requires clock-independent
    // clearing.  It drives the outputs directly, so no isolation gates exist.
    ...
    // Internal state is cleared on both release edges before core_rst_n rises.
    always_ff @(posedge clk) begin
        if (!core_rst_n) begin
            pending_q    <= '0;
            epoch_gray_q <= '0;
