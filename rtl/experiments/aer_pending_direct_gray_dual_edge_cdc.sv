...
// P9-DE experiment: P8-DG-SCR transport with a dual-edge request synchronizer.
//
// The metastability-catching stage samples on the rising edge and the second
// stage samples on the falling edge.  The synchronous core still updates only
// on rising edges.  This gives the second stage roughly half a cycle of
// resolution time, then another half cycle before the core consumes the value.
// It is an explicit latency/MTBF/physical-buffer trade-off, not an assumption
// that the ordinary same-edge two-flop synchronizer is free.
module aer_pending_direct_gray_dual_edge_cdc (
    input  logic        clk,
    input  logic        rst_n,
